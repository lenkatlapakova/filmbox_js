const filmy = [
	...
	{
		id: 's-certy-nejsou-zerty',
		nazev: 'S čerty nejsou žerty',
		plakat: {
			url: 'https://image.pmgstatic.com/cache/resized/w420/files/images/film/posters/164/849/164849539_309121.jpg',
			sirka: 525,
			vyska: 751,
		},
		ochutnavka: 'Když se kolem sebe rozhlédneme, vidíme, že ještě dneska spoustě lidí peklo chybí. ',
		popis:
...
const kartaNazev = document.querySelector('.card-title');
const kartapopis = document.querySelector('.card-text');
const obrazekFilmu = document.querySelector('.col-md-5');
const premiereElement = document.querySelector('#premiera');

const ziskejTextPremiery = (datumPremiery, dnyOdPremiery) => {
  if (dnyOdPremiery < 0) {
    return `Premiéra byla před ${Math.abs(dnyOdPremiery)} ${dnyOdPremiery === -1 ? 'dnem' : 'dny'}.`;
  } else {
    const dnyText = dnyOdPremiery === 1 ? 'den' : (dnyOdPremiery >= 2 && dnyOdPremiery <= 4) ? 'dny' : 'dní';
    return `Premiéra bude za ${dnyOdPremiery} ${dnyText}.`;
  }
};
...
if (detailFilmu) {
  ...
  filmy.forEach((film) => {
    if (idFilmu === film.id) {
      kartaNazev.innerText = film.nazev;
      kartapopis.innerText = film.popis;
      kartapopis.style.whiteSpace = 'pre-line';
      obrazekFilmu.innerHTML = `<img
        src="${film.plakat.url}"
        alt="plakát"
      ...
      let dnyOdPremiery = dayjs(datumPremiery).diff(dayjs(), 'days');
      premiereElement.innerText = ziskejTextPremiery(datumPremiery, dnyOdPremiery);

      detailFilmu.style.width = '100vh';
    }
  });
}
...
const prvkyHvezd = document.querySelectorAll('.fa-star');

let pocetHvezd = 0;

const spocitej = (pocet) => {
	console.log('Vstup počet: ' + pocet);

	prvkyHvezd.forEach((hvezda, index) => {
		console.log('Hvězda :' + hvezda);
...
const zvyrazniKlik = (udalost) => {
	console.log('klik');
	pocetHvezd = Number(udalost.target.textContent);
	console.log(pocetHvezd);
	spocitej(pocetHvezd);
};

const zvyrazniVstup = (udalost) => {
	console.log('vstup');
	spocitej(Number(udalost.target.textContent));
};

const zvyrazniOdejdi = () => {
	spocitej(pocetHvezd);
};

prvkyHvezd.forEach((hvezda) => {
	hvezda.addEventListener('click', zvyrazniKlik);
	hvezda.addEventListener('mouseenter', zvyrazniVstup);
	hvezda.addEventListener('mouseleave', zvyrazniOdejdi);
});
